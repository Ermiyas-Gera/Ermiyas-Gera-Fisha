# 👨‍💻 **Ermiyas Gera Fisha**

**Cybersecurity Leader | VAPT Manager | Red Team & SOC Expert | Ethical Hacker**

📍 Addis Ababa, Ethiopia
📧 [ermiyasgera@gmail.com](mailto:ermiyasgera@gmail.com)
📞 +251 938 047 394
🔗 [LinkedIn](https://www.linkedin.com/in/ermiyasgera/)

---

## 🛡️ **About Me**

I am a cybersecurity professional with **11+ years of experience** across **Vulnerability Assessment & Penetration Testing (VAPT), SOC Operations, Incident Response, and Security Engineering**.

Currently, I lead **offensive security operations** at **Bank of Abyssinia**, where I design, execute, and mature enterprise-level **red team, VAPT, and security monitoring programs**.

I am passionate about **building secure digital ecosystems, training future defenders, and staying ahead of adversaries**.

---

## 🎯 **Career Objective**

To actively contribute to the creation of a **secure, resilient, and threat-aware digital environment** by applying advanced cybersecurity strategies, offensive security testing, and security operations excellence.

---

## 🧠 **Core Competencies**

* Vulnerability Assessment & Penetration Testing (Web, Mobile, Network)
* Red Team Operations & Adversary Simulation
* SOC Operations & Threat Detection
* SIEM & SOAR Engineering
* Incident Response & Threat Hunting
* Social Engineering & Phishing Simulations
* Security Architecture & Risk Management
* Compliance (PCI DSS, ISO, NIST)

---

## 🏢 **Professional Experience**

### 🏦 **Bank of Abyssinia**

#### **Manager – Vulnerability Assessment & Penetration Testing**

📅 **Feb 2025 – Present | Addis Ababa**

* Led penetration tests on **20+ web & mobile applications**, reducing security risk by **40%**
* Executed red-team style attack simulations uncovering **30+ critical vulnerabilities**
* Ran phishing & social engineering campaigns with **67% engagement rate**
* Mentored and led **5 security analysts**, improving team productivity by **25%**

---

#### **Acting Manager – VAPT**

📅 **Dec 2024 – Feb 2025**

* Established the **VAPT Office from scratch**
* Recruited and trained **5 analysts**
* Developed standardized testing procedures, reducing assessment time by **20%**
* Conducted phishing campaigns that exposed **15+ high-risk gaps**

---

#### **Senior SOC Analyst**

📅 **Oct 2022 – Dec 2024**

* Implemented and managed:

  * **IBM QRadar SIEM**
  * **IBM SOAR**
  * **Wazuh**
  * **Kaspersky**
  * **OSSEC**
* Reduced incident response time by **35%**
* Built custom parsers improving alert accuracy by **50%**
* Led response to **15+ major security incidents** with **zero data loss**

---

#### **SOC Analyst**

* Handled **500+ alerts monthly** with **89% resolution rate**
* Built **50+ dashboards, correlation rules & reports**

---

#### **Senior IT Security Expert**

📅 **Jul 2018 – Jul 2019**

* Performed VA on **50+ applications**
* Reduced critical risks by **45%**
* Influenced **20% increase in security budget**

---

#### **IT Security Expert**

* Maintained **100% PCI DSS compliance**
* Identified and fixed **100+ vulnerabilities**

---

#### **Junior Web Application Security Officer**

* Assessed **50+ applications**
* Reduced vulnerabilities by **30%**

---

### 🧑‍🏫 **Rift Valley University**

**Part-Time Instructor**
📅 **Aug 2014 – Jun 2016**

* Trained **1,000+ students** in:

  * Cybersecurity
  * Programming
  * Networking
* **80% certification success rate**

---

## 🎓 **Education**

🎓 **MSc – Information Systems**
Addis Ababa University

🎓 **BSc – Computer Science**
Addis Ababa University

🎓 **Cybersecurity and Digital Forensics Student**
International Cybersecurity & Digital Forensics Academy



---

## 📜 **Certifications & Professional Training**

* **EC-Council** – CEH, CSA (SOC Analyst)
* **ICSI** – CNSS (Network Security Specialist)
* **ISC²** – Certified in Cybersecurity (CC)
* **ISO 27035** – Incident Handling
* **KPMG** – QRadar SIEM & SOAR
* **SOCradar** – Threat Intelligence & Dark Web
* **TCM Security** – Help Desk, Python, Linux
* **Cornerstone** – Project Management & MEAL
* **Udemy** – Ethical Hacking, SOC, Python, NIST, Network Security, etc
* **Many advanced cybersecurity trainings**

---

## 🛠️ **Tools & Technologies**

`Burp Suite` · `Metasploit` · `Nessus` · `SQLMap` · `Wireshark` · `Nmap` . 'Acunetix' . 'Rapid 7 VM' . 'ZAP proxy'
`IBM QRadar` · `IBM SOAR` · `Wazuh` · `OSSEC` · `Kaspersky` . 'Defectdojo'
`Linux` · `Python` . 'HTML' . 'CSS' . 'JavaScript' . 'kali linux' . 'SQL'

---

## 🧩 **What I Do on GitHub**

---

## 📫 **Let’s Connect**

If you're interested in **Security Consultation, security collaboration, red team projects, Vulnerability Assessment and Penetration Testing or SOC engineering**, feel free to reach out:

📧 **[ermiyasgera@gmail.com](mailto:ermiyasgera@gmail.com)**
🔗 **LinkedIn: [https://linkedin.com](https://www.linkedin.com/in/ermiyasgera/)**

